...
export default function SellerSignupForm({ onSuccess }) {
  ...
  const { user, session, refreshProfile } = useContext(SupabaseAuthContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Success! Call onSuccess callback if provided, then redirect to vendor dashboard
      if (onSuccess) {
        onSuccess(data);
      } else {
        // If backend signals this vendor already exists, show message and still navigate
        if (data?.existing) {
          setExistingVendor(data.vendor || true);
        }

        // Refresh profile so role='vendor' is available to RequireRole
        try {
          await (refreshProfile ? refreshProfile(user.id) : Promise.resolve());
        } catch (e) {
          // ignore refresh errors
        }

        // Redirect to vendor dashboard (give user a brief moment to see the banner)
        setTimeout(() => navigate('/dashboard/vendor'), 700);
      }
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {existingVendor && (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded">
          <p className="text-sm text-blue-900">A seller account already exists for your user. You can go to your vendor dashboard to continue onboarding or manage your store.</p>
          <div className="mt-2">
            <Button onClick={() => navigate('/dashboard/vendor')}>Go to Dashboard</Button>
          </div>
        </div>
      )}
      <div>
